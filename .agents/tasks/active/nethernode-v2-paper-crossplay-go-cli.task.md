...
- First live check found the installed `/opt/nethernode/scripts/plugins-sync.sh` inferred `/opt/nethernode` instead of tracked app `/opt/nethernode/app`; `nethernode plugins list` therefore could not find the manifest. Corrected script root auto-detection before sync. That correction also needed explicit installed-volume detection so the manifest comes from `/opt/nethernode/app` while plugin data stays at `/opt/nethernode/data/minecraft`; no runtime/data/backups were changed during diagnosis.
...
- Live verification: aux pulled `7b41342` without overwriting its `server/runtime.env` override (`MINECRAFT_VERSION=26.1.2`). The host lacks Go, so `ops/install-server-cli.sh` retained its legacy wrapper while installing the current scripts; `nethernode plugins list` then resolved manifest `/opt/nethernode/app/server/plugins.manifest` and volume `/opt/nethernode/data/minecraft/plugins` correctly.
- Sync installed SkinsRestorer `15.12.4` and refreshed Geyser to `2.11.0`, ViaVersion to `5.11.0-SNAPSHOT+1037`, and ViaBackwards to `5.11.0-SNAPSHOT+611` under the pre-existing `latest` policy. It removed only superseded Via jars. A prior `nethernode save-server` completed `save-all flush`; restart used `docker compose restart minecraft`, not `down` or a backup operation.
- Post-restart evidence: container healthy; Paper `26.1.2` started in `31.322s`; SkinsRestorer enabled, registered its Floodgate skin listener, and generated its config as `ec2-user:ec2-user` (not root-owned). RCON returned `0/5`. mcstatus.io after cache expiry reported Java `25565` and Bedrock `19132` both `online=true`. The exact five pre-sync backup archives retained identical names, sizes, and timestamps.

### Post-V2 maintenance - CLI status hardening (2026-07-11)

- Failure: installed Go CLI queried mcstatus.io with `localhost:25565` and `localhost:19132`, which API rejects because `localhost` means the API host, not EC2. Direct host TCP RCON could reset even though `docker exec nethernode-minecraft rcon-cli list` succeeded.
...
- Live sync: commit `92a7ef0` pushed to `dev`; GitHub Actions CI run `29180615678` passed. Aux pulled the commit, kept its local `server/runtime.env` override (`MINECRAFT_VERSION=26.1.2`), and received `/opt/nethernode/data/minecraft/plugins/NetherNodeAdmin.jar` with matching SHA-256 and `ec2-user:ec2-user` mode `644`. Backup inventory SHA-256 remained `46367a8c19915a516ec313c15512538ce1141f9c8a7513a19a32c721c9b5229b`; world `level.dat` remained present; container stayed `running healthy`; `Player6` stayed online. No restart occurred, so `/plugins` correctly does not list `NetherNodeAdmin` yet. Activation is intentionally pending a player-safe restart.
