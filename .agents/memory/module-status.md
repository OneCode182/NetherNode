...
| Local controller | Locally verified; auxiliary sync pending | Bash works from fish; PowerShell/BAT support AWS-polled status and SSH remote actions. Seven controller mock checks, full local gates, IaC validation, and read-only auxiliary status/data-integrity checks passed. Atomic commits, push, auxiliary Git sync, and CI verification remain pending. |
