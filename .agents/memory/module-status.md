...
| Go CLI | Status hardening in progress | `internal/{rcon,backup,compose,mcstatus,cli,opsjson,serverprops}` build/vet/test offline; status uses public `MINECRAFT_STATUS_HOST`, container `rcon-cli` first, TCP fallback, and terminal-aware color. `ops/install-server-cli.sh` uses local Go, image extract, or Docker Go builder so EC2 does not regress to shell wrapper. |
